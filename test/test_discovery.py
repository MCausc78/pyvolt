...
async def test_bots():
    ...
    client = pyvolt.DiscoveryClient(
        base="http://127.0.0.1:5101/", session=ClientSession(), state=state
    )
    ...
    assert bot.usage == pyvolt.BotUsage.HIGH
    ...
    assert metadata.type == pyvolt.AssetMetadataType.IMAGE
...
async def test_servers():
    ...
    state.setup(parser=pyvolt.Parser(state))
    client = pyvolt.DiscoveryClient(
        base="http://127.0.0.1:5102/", session=ClientSession(), state=state
    )
    ...
    assert (
        server.description == "peeps is an active chatting server with memes and more"
    )
    icon = server.icon
    assert icon
    ...
    assert server.activity == pyvolt.ServerActivity.HIGH
    ...
    assert server.activity == pyvolt.ServerActivity.MEDIUM
...
async def test_themes():
    ...
    state.setup(parser=pyvolt.Parser(state))
    client = pyvolt.DiscoveryClient(
        base="http://127.0.0.1:5103/", session=ClientSession(), state=state
    )
    ...
    assert (
        theme.description
        == "Purple... based on spinfish's amethyst betterdiscord theme."
    )
