...
# Originally named DPYHTML5Translator
class PyvoltHTML5Translator(HTML5Translator):
...
class PyvoltStandaloneHTMLBuilder(StandaloneHTMLBuilder):
...
class PyvoltMessageCatalogBuilder(MessageCatalogBuilder):
    ...
    def finish(self) -> None:
        ...
        context = {
            ...
            'ctime': datetime.datetime.fromtimestamp(mktime(timestamp), datetime.timezone.utc).strftime('%Y-%m-%d %H:%M%z'),
...
def add_builders(app):
    ...
    app.set_translator('html', PyvoltHTML5Translator, override=True)
    app.add_builder(PyvoltStandaloneHTMLBuilder, override=True)
    app.add_builder(PyvoltMessageCatalogBuilder, override=True)
    ...
    try:
        ...
        injected_mro = tuple(
            base if base is not StandaloneHTMLBuilder else PyvoltStandaloneHTMLBuilder for base in original.mro()[1:]
        )
        new_builder = type(original.__name__, injected_mro, {'name': 'readthedocs'})
        app.set_translator('readthedocs', PyvoltHTML5Translator, override=True)
