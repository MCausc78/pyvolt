...
if typing.TYPE_CHECKING:
    ...
    from .builder import PyvoltHTML5Translator
...
def visit_attributetable_node(self: PyvoltHTML5Translator, node: attributetable) -> None:
    class_ = node['python-class']
    self.body.append(f'<div class="py-attribute-table" data-move-to-id="{class_}">')


def visit_attributetablecolumn_node(self: PyvoltHTML5Translator, node: attributetablecolumn) -> None:
    self.body.append(self.starttag(node, 'div', CLASS='py-attribute-table-column'))


def visit_attributetabletitle_node(self: PyvoltHTML5Translator, node: attributetabletitle) -> None:
    self.body.append(self.starttag(node, 'span'))


def visit_attributetablebadge_node(self: PyvoltHTML5Translator, node: attributetablebadge) -> None:
    attributes = {
...
def visit_attributetable_item_node(self: PyvoltHTML5Translator, node: attributetable_item) -> None:
    self.body.append(self.starttag(node, 'li', CLASS='py-attribute-table-entry'))


def depart_attributetable_node(self: PyvoltHTML5Translator, _node: attributetable) -> None:
    self.body.append('</div>')


def depart_attributetablecolumn_node(self: PyvoltHTML5Translator, _node: attributetablecolumn) -> None:
    self.body.append('</div>')


def depart_attributetabletitle_node(self: PyvoltHTML5Translator, _node: attributetabletitle) -> None:
    self.body.append('</span>')


def depart_attributetablebadge_node(self: PyvoltHTML5Translator, _node: attributetablebadge) -> None:
    self.body.append(' </span>')


def depart_attributetable_item_node(self: PyvoltHTML5Translator, _node: attributetable_item) -> None:
    self.body.append('</li>')
...
def process_attributetable(app: Sphinx, doctree: nodes.Node, _fromdocname: str) -> None:
    env = app.builder.env
